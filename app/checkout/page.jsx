...
import countryList from 'react-select-country-list'; // External library for countries
import Preloader from "@/components/Preloader";
import { loadRazorpayScript, createOrderOnServer } from "../utils/razorpayUtils"; // Import utility functions for Razorpay
...
const CheckoutPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (validateForm()) {
      ...
      // Save form data to Firestore before proceeding to payment
  ...
  const handleRazorpayPayment = async () => {
    // Call a function to load the Razorpay script dynamically
    ...
    // Create an order on your backend server and get the order_id
    const orderData = await createOrderOnServer(user.uid); // Create order function

    const options = {
      key: process.env.NEXT_PUBLIC_RAZORPAY_KEY_ID, // Razorpay Key ID from the environment
      amount: orderData.amount, // Amount in paise
      currency: orderData.currency,
      name: "MaaDiy",
      description: "Subscription Payment",
      order_id: orderData.id, // Get the order_id from the server
      prefill: {
      ...
      handler: async (response) => {
        // On successful payment, store the payment data in Firestore
        await setDoc(doc(db, "users", user.uid, "payments", response.razorpay_payment_id), {
