...
export const POST = async (req) => {
    try {
        ...
        if (type === 'signup') {
            const userRef = doc(db, 'users', email); // Corrected
            const docSnap = await getDoc(userRef); // Corrected
            if (docSnap.exists()) {
                return new Response(JSON.stringify({ success: false, error: 'User already exists' }), { status: 400 });
            }
            const hashedPassword = await bcrypt.hash(password, 10);
            await setDoc(userRef, { // Corrected
                email,
            ...
            const user = docSnap.data(); // Corrected
        ...
        console.error('API Error:', error); // Log the error to diagnose the issue
