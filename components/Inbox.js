    ...
    const fetchWebhookStatus = async () => {
      try {
        ...
        if (userDocSnap.exists()) {
          ...
          const isWebhookVerified = userData.webhook?.verified || false;
          setIsWebhookConnected(isWebhookVerified);
        }
      } catch (error) {
        ...
        const unsubscribe = onSnapshot(phoneNumbersCollection, (snapshot) => {
          ...
          snapshot.forEach((phoneDoc) => {
            ...
            onSnapshot(conversationMessagesQuery, (conversationSnapshot) => {
              ...
              conversationSnapshot.forEach((msgDoc) => {
                const timestamp = msgDoc.data().timestamp?.seconds
                  ? msgDoc.data().timestamp.seconds * 1000
                  : Date.now();
                ...
                if (!conversationNumbers.has(phoneNumber)) {
                  ...
                  // Update to the latest timestamp
                  conversationNumbers.get(phoneNumber).timestamp = Math.max(
                    conversationNumbers.get(phoneNumber).timestamp,
                    timestamp
                  );
                }
                conversationNumbers.get(phoneNumber).count += 1;
      ...
      setConversations((prev) => {
        const updatedConversations = [
          ...
          [
            ...
            {
              count:
                (prev.find(([num]) => num === number)?.[1].count || 0) +
                querySnapshot.size,
              timestamp: Date.now(),
            },
          ],
            ...
            <div className="ml-4 flex items-center">
              {isWebhookConnected ? (
                <>
                  ...
                  <span
                    className="material-icons ml-2 text-green-500"
                    style={{ fontSize: "28px" }}
                  >
                    lock
                  </span>
                ...
                <>
                  <span className="text-red-500 text-lg pt-2 font-semibold">
                    Not Connected
                  </span>
                  <span
                    ...
                    <div className="flex-shrink-0">
                      ...
                      >
                        <path
                          ...
                          strokeWidth="2"
                          d="M16 4h6v6m-9 9l-4 4m0 0l-4-4m4 4V10"
                        />
                      </svg>
                    </div>
                  </Link>
